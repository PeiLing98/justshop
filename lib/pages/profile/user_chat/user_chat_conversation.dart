...
class UserChatConversation extends StatefulWidget {
  final Chat chat;
  final Store store;
  const UserChatConversation(
      {Key? key, required this.chat, required this.store})
      : super(key: key);

  @override
  _UserChatConversationState createState() => _UserChatConversationState();
}

class _UserChatConversationState extends State<UserChatConversation> {
                ...
                if (snapshot.hasData) {
                  ...
                  matchedChat = chat?.where((chat) {
                    return chat.user1 == user?.uid &&
                        chat.user2 == widget.store.storeId;
                  }).toList();
                      ...
                      Expanded(
                        ...
                        child: Padding(
                            ...
                            child: matchedChat!.isEmpty
                                ...
                                : ListView.builder(
                                    itemCount: matchedChat[0].message.length,
                                    itemBuilder: (context, index) {
                                      return Row(
                                        mainAxisAlignment: matchedChat?[0]
                                                    .message[index]['sendBy'] ==
                                                widget.chat.user1
                                        ...
                                        if (widget.chat.chatId == "") {
                                          if (matchedChat!.isEmpty) {
                                            ...
                                            await DatabaseService(
                                                    uid: user?.uid)
                                                .updateMessage(
                                                    matchedChat[0].chatId,
                                                    conversation);
                                            controller.clear();
                                          }
                                        } else {
                                          await DatabaseService(uid: user?.uid)
                                              .updateMessage(widget.chat.chatId,
                                                  conversation);
                                          controller.clear();
                                        }
                                      },
